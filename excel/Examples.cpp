...
inline float getPara(string paraName)
{
    ...
    if(xls.Load(filename.c_str()))
        ...
        return -1;
    BasicExcelWorksheet *sheet=xls.GetWorksheet(0);
    int paraCount=sheet->GetTotalRows()-1;
    cout<<"parameters count:"<<paraCount<<endl;

    for (int i=0;i<=paraCount;i++)
    {
        if(paraName.compare(sheet->Cell(i,0)->GetString())==0)
        {
//            cout<<sheet->Cell(i,1)->GetDouble();
            return sheet->Cell(i,1)->GetDouble();
        }
    }
    return -1;
}
...
{
    string paraName="regionMaxP";
    cout<<getPara(paraName);
}
